...
5. Esittelyviestin jälkeen se kysyy sinulta, mitä IOS:ia haluat käyttää sovelluksessa. Jos olet [asentanut cIOS:in](cios), käytä `IOS249`, tai muuten käytä `IOS58`. If the former gives an `Exception DSI occurred!` error, press Reset on the Wii console, launch it again, then try `IOS250`. It might take a couple attempts to reload the IOS.
